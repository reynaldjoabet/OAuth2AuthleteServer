  ...
  val routes: AuthedRoutes[SessionUser, F] = AuthedRoutes.of[SessionUser, F] {
    ...
    case req @ POST -> Root / "authorization" / "decision" as session =>
      req
        ...
        /**
          * The flag to indicate whether the client application has been granted permissions by the
          * user.
          */
        ...
        /**
          ...
          * request.
          */
        val mIdTokenClaims: Map[String, Object] = ???
        ...
        lazy val authorizationFailRequest: AuthorizationFailRequest = AuthorizationFailRequest()
          .setReason(Reason.DENIED)
          .setTicket("ticket")

        lazy val authorizationIssueRequest: AuthorizationIssueRequest = AuthorizationIssueRequest()

        // if(true)//user gave approval
        Async[F].ifM(Async[F].delay(true))(
    ...
    authleteService
      ...
      .flatMap { resp =>
        resp.getAction() match {
          case AuthorizationFailResponse.Action.INTERNAL_SERVER_ERROR =>
            Status.InternalServerError()
          case AuthorizationFailResponse.Action.BAD_REQUEST => Status.BadRequest()
          case AuthorizationFailResponse.Action.LOCATION    => Status.TemporaryRedirect(???)
          case AuthorizationFailResponse.Action.FORM        => ???
        }
